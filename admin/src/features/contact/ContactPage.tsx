...
export default function ContactPage() {
  ...
  const unreviewedQuery = useContactSubmissions({ isReviewed: false, perPage: 1 })
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Message',
        render: (row: ContactSubmission) => (
          <span className="text-sm text-muted-foreground line-clamp-2 max-w-[350px]">
            {row.message}
          </span>
        ),
      },
      ...
      },
      {
        ...
        header: 'Reviewed At',
        render: (row: ContactSubmission) => (row.reviewedAt ? formatDate(row.reviewedAt) : '—'),
      },
      {
  ...
  return (
    <div className="space-y-6">
      <PageHeader heading="Contact Submissions" text="Manage customer inquiries and feedback." />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-amber-600">Unreviewed</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{reviewedQuery.data?.total ?? 0}</div>
          </CardContent>
        </Card>
      </div>
