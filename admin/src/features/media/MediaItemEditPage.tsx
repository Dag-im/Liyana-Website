...
import { useMediaItems, useUpdateMediaItem } from './useMedia'

export default function MediaItemEditPage() {
  const { folderId = '', itemId = '' } = useParams()
  const location = useLocation()
  const navigate = useNavigate()

  const itemsQuery = useMediaItems(folderId, { page: 1, perPage: 500 })
  const updateMutation = useUpdateMediaItem(folderId, itemId)

  const item = useMemo(
    () => itemsQuery.data?.data.find((entry) => entry.id === itemId),
    [itemId, itemsQuery.data?.data]
  )
  ...
  useEffect(() => {
    if (!item) return
    setTitle(item.title)
    setMode(isYouTubeUrl(item.url) ? 'youtube' : 'upload')
    setUrl(item.url)
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader heading="Edit Media Item" text="Update media item details.">
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <Tabs value={mode} onValueChange={(value) => setMode(value as 'upload' | 'youtube')}>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button
                disabled={
                  updateMutation.isPending || !url || (mode === 'youtube' && !isYouTubeUrl(url))
                }
                type="submit"
              >
                Save Changes
